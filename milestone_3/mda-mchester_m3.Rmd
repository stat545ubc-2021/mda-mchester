...
2. In benign tumours, are the differences of means between categories of ```symmetry_mean``` (low, medium, high) and ```perimeter_mean``` statistically significant?
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
```{r symmetry plot, fig.align = "center"}
...
<!----------------------------------------------------------------------------->
```{r symmetry reordered, fig.align = "center"}
cancer_sample %>% #create categorical variable
  filter(diagnosis == "B") %>%
  mutate(category_symmetry = cut(symmetry_mean, 
                        breaks = 3,
...
The ```symmetry_mean``` categories were reordered to improve visualiztion and are now based on highest ```perimeter_mean```. Interestingly, the boxplot now displays that "low" symmetry values produce the highest ```perimeter_mean``` values in benign tumours, on average. With greater sample size, an interesting follow up question here would be whether highly symmetrical tumours with lower perimeter means are more likely to be malignant.

<!-------------------------- Start your work below ---------------------------->

**Task Number**: 2

```{r symmetry other, fig.align = "center"}
cancer_sample %>% #create categorical variable
  filter(diagnosis == "B") %>%
  mutate(category_symmetry = cut(symmetry_mean, 
                        breaks = 3,
  ...
  labs(x = "Symmetry Category", #add labels
    ...
    scale_colour_discrete(limits = c("B"),
                          ...
                          name = "Diagnosis")
```
The "high" and "medium" ```symmetry_mean``` categories have now been grouped together to compare to the "low" category. With these combined, the difference between group means appears insignificant. Again, an interesting follow up question here would be whether or not this pattern holds true for malignant diagnoses.
...
**Research Question**: Is ```area_mean``` a significant predictor of a malignant diagnosis?
...
<!-------------------------- Start your work below ---------------------------->
```{r correlation area_mean and diagnosis, fig.align = "center"}
cancer_sample %>%
...
Therefore, with a t-value greater than 2 and a p-value less than 0.05, the difference in ```area_mean``` by diagnosis is significant
...
I have decided to use the  ```broom``` package to eloquently display t-values for the differences of means. These values can be found in the "statistic" column. The corresponding p-values are displayed in the "p.value" column.

```{r broom}
broom::tidy(lm(area_mean ~ diagnosis, data = cancer_sample))
```
I have also decided to use the ```broom ``` package to make predictions on ```area_mean``` based on diagnosis. An interesting future question could look at prediction models based on a greater number of variables.
